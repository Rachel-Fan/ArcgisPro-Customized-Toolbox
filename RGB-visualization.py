...
class ImageViewer:
    def __init__(self, root_folder_template, additional_folder_template1, additional_folder_template2, additional_folder_template3, additional_folder_template4, states):
        ...
        self.additional_folder_template4 = additional_folder_template4  # Folder for GLCM
        self.states = states
    ...
    def update_folders(self, state):
        ...
        self.additional_folders = [
            ...
            self.additional_folder_template4.format(State=state)   # pa-sam-glcm (4th window)
        ]
        self.filenames = sorted(os.listdir(self.image_folder))
    ...
    def show_current_image(self):
        ...
        if self.filenames:
            ...
            # Load images from the 4 additional folders and display them with labels below
            additional_labels = ["Unet", "sam-hq", "pa-sam", "pa-sam-glcm"]
            ...
            # Second row for pa-sam-glcm (below sam-hq in column 1)
            glcm_label = tk.Label(self.additional_images_frame)
            glcm_label.grid(row=2, column=1, padx=10, pady=10)
            self.show_image(glcm_label, self.additional_folders[3], filename, size=(300, 300))

            # Add label below the glcm image
            glcm_text_label = tk.Label(self.additional_images_frame, text=additional_labels[3])
            glcm_text_label.grid(row=3, column=1, padx=10, pady=5)
...
def open_corresponding_image():
    ...
    additional_folder_template4 = r'D:\Seagrass\training_result\RGBChannel\{State}\glcm'  # Folder for pa-sam-glcm

    app = ImageViewer(root_folder_template, additional_folder_template1, additional_folder_template2, additional_folder_template3, additional_folder_template4, states)
    app.run()
