...
input_folder = r'C:\Users\GeoFly\Documents\rfan\Seagrass\Example_image\images'
output_folder = r'C:\Users\GeoFly\Documents\rfan\Seagrass\Example_image\glcm'
...
properties = ['contrast', 'correlation', 'energy', 'homogeneity', 'entropy']  # List of texture properties to compute

def compute_texture_image(image, distances, angles):
    ...
    for i in range(height):
        for j in range(width):
            ...
            texture_images['contrast'][i, j] = graycoprops(glcm, 'contrast').mean()
            texture_images['correlation'][i, j] = graycoprops(glcm, 'correlation').mean()
            texture_images['energy'][i, j] = graycoprops(glcm, 'energy').mean()
            texture_images['homogeneity'][i, j] = graycoprops(glcm, 'homogeneity').mean()

            # For entropy, calculate it separately
            glcm_entropy = -np.sum(glcm * np.log2(glcm + (glcm == 0)))
            texture_images['entropy'][i, j] = glcm_entropy
...
def process_images(input_folder, output_folder, distances, angles):
    ...
    for filename in image_files:
        ...
        try:
            ...
            texture_images = compute_texture_image(image_gray_uint8, distances, angles)

            # Create output subfolder named after the image file
            subfolder_path = os.path.join(output_folder, os.path.splitext(filename)[0])
            os.makedirs(subfolder_path, exist_ok=True)

            # Save the texture images for each property
            for prop in properties:
                output_path = os.path.join(subfolder_path, f"{os.path.splitext(filename)[0]}_{prop}.png")
